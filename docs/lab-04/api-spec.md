...
- `counts.myAssigned` = count of Tickets where `assigneeId = current user` and `status` is in the open-work set `{New, Open, InProgress, WaitingForRequester, Reopened}` (BR-14) — i.e., `Closed` and `Cancelled` Tickets assigned to the user do not count toward "My Assigned".
...
- `recentTickets` = the 5 most recently `updatedAt` Tickets where `assigneeId = current user` **or** `assigneeId IS NULL`, excluding `Cancelled`, newest first.
