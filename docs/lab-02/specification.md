...
19. **Multer-before-auth middleware order**: Endpoints that accept `multipart/form-data` (`POST /api/tickets` and `POST /api/tickets/:ticketNumber/attachments`) use `upload.array()` before `requesterContext` in the middleware chain. This means multer parses and validates file size/type (413/415 rejection) before the auth check runs. The trade-off is that an unauthenticated request can trigger file parsing (consuming memory for the buffer) before being rejected by auth. This is accepted in Lab 2 because: (a) the scope is small and non-production, (b) multer's size limit (5 MB) caps memory usage per request, and (c) rejecting invalid files early avoids wasting a DB query on auth for obviously-bad requests. In Lab 3 (real auth), this order should be revisited — placing auth before multer prevents unauthenticated file parsing entirely.

20. **Playwright `fullPage` screenshot capture requires explicit waits before capture** — Playwright's `fullPage: true` captures the entire document height by scrolling. When a page has async data fetching (e.g., `GET /api/tickets/:ticketNumber`), `waitForTimeout(N)` does not guarantee the React component tree has finished re-rendering after the API response arrives. This can cause the screenshot to capture a mid-transition state where layout has shifted (e.g., the sticky header appears at y=410 instead of y=0). **Mitigation**: All screenshot scripts must use explicit waits before capture: (1) `waitForLoadState('networkidle')`, (2) `waitForSelector('h1')` for the page heading, (3) `waitForSelector` for a content-specific selector that confirms data has rendered (e.g., `[class*='fieldGrid']` for Ticket Detail, `tr` for My Tickets). Verified by capturing 3 consecutive screenshots of Ticket Detail — all showed header at y=0-55 (deterministic) vs the previous `waitForTimeout(3000)` approach which produced a header at y=410 (one-off race condition). This applies to all pages with async fetch: Ticket Detail, Create Ticket (post-submit), and any future page.
21. **lab2/08 reference-data-api scope** — The originally flagged issues (wrapped response format, auth middleware for categories/related-systems) were all resolved in lab2/05 (see items 7–8 above). lab2/08 scope was therefore changed to cleanup: (a) Added inactive-filtering test coverage for both `GET /api/categories` and `GET /api/related-systems` — each test creates an `isActive=false` record via Prisma, asserts it is excluded from the API response, and cleans up the record. This closes a regression-risk gap where removing the `isActive: true` filter in the handler would go undetected. (b) Deleted `client/src/api.ts` — a Lab 1 leftover (`checkSystem()` function using raw `fetch` without auth headers, parsing the old bare-array response shape). No runtime component imported this file; only a Lab 1 test (`tests/lab-01/App.test.tsx`) dynamically imported it with mocked fetch. The Lab 1 `checkSystem` test block (2 tests) was removed; the 3 `App` tests in the same file were retained. (c) Fixed a pre-existing TS2322 in `AttachmentSection.tsx` — the upload button used `busy={isUploading}` (non-existent prop on `Button`) which compiled only because vitest ignores type errors. Changed to `variant={isUploading ? 'busy' : 'primary'}` which correctly uses Button's variant-based busy state (spinner + busyLabel). Added a new test to verify the "Uploading…" busyLabel appears during upload.
22. **Every branch must run `tsc --noEmit` before closing, not just `vitest run`** — Vitest transpiles TypeScript on-the-fly and does not perform full type-checking, so it silently ignores type errors like non-existent props (e.g., the `busy` prop on `Button` in lab2/07 AttachmentSection, which compiled fine under vitest but was a TS2322). `tsc --noEmit` catches these errors. The branch-close checklist should include: (1) `cd server && npx tsc --noEmit`, (2) `cd client && npx tsc --noEmit`, (3) `vitest run` for both. This was discovered in lab2/08 when the deleted `api.ts` file required a type-check pass to confirm no stale imports remained, which surfaced the pre-existing AttachmentSection type error as a side-effect.
