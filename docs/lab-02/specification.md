...
- [ ] Every AC-01 through AC-24 has at least one passing, traceable automated test (see `tests.md`).
...
7. **`GET /api/dev-requesters` follows `api-spec.md`'s wrapped response format** (`{ requesters: [...] }`). The existing `GET /api/categories` endpoint from Lab 1 does not match this pattern (returns a bare array) — this inconsistency is out of scope for lab2/04 and is flagged for correction in lab2/08 (reference data API) when categories/related-systems endpoints are properly implemented per `api-spec.md`.
8. **The `X-Dev-Requester-Id` auth header enforcement is not yet applied to `GET /api/categories`** (Lab 1 implementation predates this requirement) — also flagged for lab2/08.
9. **`categories.test.ts` (Lab 1) uses hardcoded category IDs (1–4)** which drift from actual DB state after repeated seed/reset cycles during development. This is a known test-fragility issue predating Lab 2, flagged for cleanup when Lab 1 tests are next touched — not blocking for any Lab 2 branch.
